...
export default function BrandGrid(): JSX.Element {
  ...
  return (
    <>
      ...
      <div className="space-y-6 p-6">
        <ComponentCard title="Pilih Brand untuk melihat Store">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.length === 0 && !loading ? (
              ...
              filtered.map(b => (
                <div key={b.id} className="border rounded-lg p-4 flex flex-col justify-between bg-white hover:shadow transition-shadow">
                  ...
                  <div className="mt-4 flex items-center justify-start gap-2">
                    <button
                      onClick={() => navigate(`/superadmin/brands/${b.id}/stores`)}
                      className="px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700"
                    >
                      Lihat Store
                    </button>
                    {/* <button
                      onClick={() => navigate(`/superadmin/brands/${b.id}/edit`)}
