// src/pages/BrandRecipesLanding.tsx
...
export default function BrandRecipesLanding(): JSX.Element {
  ...
  async function fetchBrands() {
    setLoading(true);
    try {
      const resp = await axios.get("/api/brands", { params: { limit: 500 } });
  ...
  return (
    <>
      <PageMeta title="Brands — Recipes" description="Daftar brand untuk manajemen recipes" />
      <PageBreadcrumb pageTitle="Brands — Recipes" />

      <div className="space-y-6 p-6">
        <ToastContainer position="top-right" autoClose={2000} />
        <ComponentCard title="Pilih Brand untuk melihat Recipes">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            {/* optional actions area (ke depannya bisa ditambahkan tombol) */}
            {/* <div className="flex gap-2">
              <button onClick={() => setQ("")} className="px-3 py-2 border rounded bg-white hover:bg-gray-50 text-sm">Reset</button>
            </div> */}
          </div>
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {/* optionally show color badge */}
                      {/* {b.color && (
                        <div className="mt-3">
                          <span className="inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-medium" style={{ backgroundColor: b.color, color: '#fff' }}>
                            {b.color}
                          </span>
                        </div>
                      )} */}
                  ...
                  <div className="mt-4 flex items-center justify-start gap-2">
                    <button
                      onClick={() => navigate(`/superadmin/brands/${b.id}/recipes`)}
                      className="px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700"
                    >
                      Lihat Recipes
                    </button>

                    {/* kept for future: navigate to items if needed */}
                    {/* <button
                      onClick={() => navigate(`/brands/${b.id}/items`)}
